;; Editted by Matthew Perez
;; Hunters, sheep, wovles all give same amount of energy when eaten
;; hunters and wolves can eat all agents
breed [sheep a-sheep]
breed [wolves wolf]
...
to go
  ...
  ask turtles [  ;; ask both wolves and sheep
    ...
    eat            ;; sheep eat grass, wolves eat sheep, humans and wolves eat each other (check energy)
    reproduce      ;; humans eat sheep
  ]
...
to eat-predators
  ifelse breed = hunters [ ;; hunter finds a wolf
    if any? wolves-here [
      let target one-of wolves-here
      if energy > [energy] of target [
        print "hunter wins"
        ask target[die]
        set energy energy + energy-gain-from-sheep
      ]
  ...
  [ ;;wolves
    if any? hunters-here [
      let target one-of hunters-here
      if energy > [energy] of target [
        print "wolf wins"
        ask target[die]
        set energy energy + energy-gain-from-sheep
...
to eat
  ifelse breed = sheep ;;check sheep. hunters and wolves both eat sheep
  [eat-grass]
  [eat-sheep eat-predators]
...
to my-update-plots
  ...
  set-current-plot-pen "hunters"
  plot count hunters * 10 ;; scaling factor so plot looks nice
