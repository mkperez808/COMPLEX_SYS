patches-own [
  chemical             ;; amount of chemical on this patch (food carrying scent)
  food                 ;; amount of food on this patch (0, 1, or 2)
  nest?                ;; true on nest patches, false elsewhere
  nest-scent           ;; number that is higher closer to the nest (temporary)
...
to go  ;; forever button
  ...
  [ if who >= ticks [ stop ] ;; delay initial departure
    ifelse color = red
    [ look-for-food  ]       ;; if color = red -> not carrying food. look for it
    [ return-to-nest ]       ;; carrying food? take it back to nest
    wiggle ;;random movement
    fd 1 ]
  diffuse chemical (diffusion-rate / 100)
  diffuse nest-scent (diffusion-rate / 100) ;; diffuse nest-scent
  ask patches
  [ set chemical chemical * (100 - evaporation-rate) / 100  ;; slowly evaporate chemical
    recolor-patch ]
  ask patches
  [ set nest-scent nest-scent * (100 - evaporation-rate) / 100  ;; slowly evaporate nest-scent
    recolor-patch ]
  tick
end
...
to return-to-nest  ;; turtle procedure
  ...
  [ ;; if at nest, drop food and head out again
    set color red
    set nest-scent 200 ;; Everytime a ant comes back to nest, reset nest-scent
    rt 180 ]
  [ set chemical chemical + 60  ;; not at nest. drop some chemical
