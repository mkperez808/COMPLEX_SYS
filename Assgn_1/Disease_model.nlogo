breed [healthy healthy-sheep]
breed [disease disease-sheep]
...
to setup
  ...
  create-healthy number-of-healthy [  ;; create the initial sheep
    setxy random-xcor random-ycor
    set color white
    set shape "sheep"
    set energy 100  ;; set the initial energy to 100
  ]
  create-disease number-of-disease [  ;; create the initial sheep
    setxy random-xcor random-ycor
    set color black
    set shape "sheep"
    set energy 100  ;; set the initial energy to 100
...
to go
  ...
  ask turtles [  ;; ask both wolves and sheep
    ...
    eat-grass            ;; sheep eat grass, wolves eat sheep
    disease-carry
    reproduce
  ]
...
to eat-grass
  ...
  if ( grass-amount >= energy-gain-from-grass ) [
    ...
    if breed = healthy ;;only healthy sheep eat grass
...
;;disease infects other healhy
to disease-carry
  if breed = disease [
    if any? healthy-here [
    let target one-of healthy-here
    ask target [
        set breed disease
        set color black
        set shape "sheep"
...
to my-update-plots
  ...
  set-current-plot-pen "disease"
  plot count disease * 10 ;; scaling factor so plot looks nice
...
to move
  forward 1
  ifelse breed = healthy
  [set energy energy - movement-cost] ;; reduce the energy by the cost of movement
  [set energy energy - movement-cost * 5] ;; disease sheep move with 5* more energy
end
...
"" ""
PENS
"disease" 1.0 0 -16777216 true "" ""
"grass" 1.0 0 -10899396 true "" ""
"healthy" 1.0 0 -13791810 true "" ""
